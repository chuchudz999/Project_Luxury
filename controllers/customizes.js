...
const customizeController = {
  getAllDocuments: async (req, res) => {
    try {
      ...
      const orders = await Orders.find({}).estimatedDocumentCount();

      if (users && categories && products && orders)
        return res.json({ users, products, categories, orders });
    } catch (error) {
      console.log(error);
    }
  },
  ...
  deleteImage: async (req, res) => {
    try {
      ...
      const filePath = path.join(
        __dirname,
        `../client/public/uploads/customizes/${image.slideImage}`
      );
